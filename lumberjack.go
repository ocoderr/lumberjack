...
type Logger struct {
	...
	// BackupPath is the file to Backup log to.  if startwith "/" is a Absolute path
...
func (l *Logger) backupName(name string, local bool) string {
	...
	return filepath.Join(l.backupDir(), fmt.Sprintf("%s-%s%s", prefix, timestamp, ext))
}
...
func (l *Logger) backupDir() string {
	if l.BackupPath != "" {
		if strings.HasPrefix(l.BackupPath, "/") {
			return l.BackupPath
		} else {
			filepath.Join(l.dir(), "/", l.BackupPath)
		}
	}
	return filepath.Join(l.dir(), "/", backupPath)
}
